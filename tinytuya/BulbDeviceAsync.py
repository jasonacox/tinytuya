# TinyTuya Bulb Device Async
# -*- coding: utf-8 -*-
"""
 Python module to interface with Tuya WiFi smart devices (Async version)
 ...
 Functions
    ...
    BulbDeviceAsync (all async)
        await set_mode(self, mode="white", nowait=False):
        await set_scene(self, scene, scene_data=None, nowait=False):
        ...
        await set_brightness_percentage(brightness=100, nowait):
        await set_colourtemp_percentage(colourtemp=100, nowait):
        result = await get_value(self, feature, state=None, nowait=False):
        result = await get_mode(self, state=None, nowait=False):
        result = await get_brightness_percentage(self, state=None, nowait=False):
        result = await get_colourtemp_percentage(self, state=None, nowait=False):
        (r, g, b) = await colour_rgb():
        (h,s,v) = await colour_hsv()
        result = await state():
        bool = await bulb_has_capability( self, feature, nowait=False ):
        await detect_bulb(self, response=None, nowait=False):
        set_bulb_type(self, bulb_type=None, mapping=None):
        set_bulb_capabilities(self, mapping):

    Inherited
        Every device function from core.DeviceAsync
"""

import colorsys

from .core import DeviceAsync, log
from .core import error_json, ERR_JSON, ERR_FUNCTION # ERR_RANGE, ERR_STATE, ERR_TIMEOUT

# pylint: disable=R0904
class BulbDeviceAsync(DeviceAsync):
    """
    ...
    def __init__(self, *args, **kwargs):
        ...
        if 'version' not in kwargs or not kwargs['version']:
            kwargs['version'] = None
        super(BulbDeviceAsync, self).__init__(*args, **kwargs)
    ...
    def hsv_to_hexvalue(h, s, v, hexformat):
        ...
        if hexformat == 'rgb8':
            ...
            return BulbDeviceAsync.rgb_to_hexvalue( r * 255.0, g * 255.0, b * 255.0, hexformat )
        elif hexformat == 'hsv16':
            # h:0-360,s:0-1000,v:0-1000|hsv|
    ...
    def hexvalue_to_hsv(hexvalue, hexformat=None):
        ...
        if hexformat == 'rgb8':
            ...
            if hexvalue_len < 14:
                # hexvalue is in rgb
                rgb = BulbDeviceAsync.hexvalue_to_rgb(hexvalue, 'rgb8')
    ...
    async def _set_values_check( self, check_values, nowait=False ):
        dps_values = {}

        # check to see which DPs need to be set
        state = await self.cached_status(nowait=nowait)
        ...
        return await self.set_multiple_values( dps_values, nowait=nowait )

    async def turn_onoff(self, on, switch=0, nowait=False):
    ...
    async def turn_on(self, switch=0, nowait=False):
        """Turn the device on"""
        return await self.turn_onoff( True, switch=switch, nowait=nowait )

    async def turn_off(self, switch=0, nowait=False):
        """Turn the device off"""
        return await self.turn_onoff( False, switch=switch, nowait=nowait )

    async def set_mode(self, mode="white", nowait=False):
        """
        Set bulb mode
    ...
    async def set_scene(self, scene, scene_data=None, nowait=False):
        """
        Set to scene mode
        ...
        return await self._set_values_check( dps_values, nowait=nowait )
    ...
    async def set_music_colour( self, transition, red, green, blue, brightness=None, colourtemp=None, nowait=False ):
        """
        ...
        return await self.set_value(self.dpset['music'], colour, nowait=nowait)

    async def set_colour(self, r, g, b, nowait=False):
        """
    ...
    async def set_hsv(self, h, s, v, nowait=False):
        """
    ...
    async def set_white_percentage(self, brightness=100, colourtemp=0, nowait=False):
        """
        ...
        return await self.set_white( b, c, nowait=nowait )

    # Deprecated.  Please use set_white_percentage() instead.
    async def set_white(self, brightness=-1, colourtemp=-1, nowait=False):
        """
    ...
    async def set_brightness_percentage(self, brightness=100, nowait=False):
        """
        ...
        b = int(self.dpset['value_max'] * brightness // 100)
        return await self.set_brightness(b, nowait=nowait)

    # Deprecated.  Please use set_brightness_percentage() instead.
    async def set_brightness(self, brightness, nowait=False):
        """
        ...
        if state['mode'] != self.DPS_MODE_COLOUR:
            ...
            return await self.set_white(brightness=brightness, colourtemp=None, nowait=nowait)
        else:
            ...
            return await self.set_hsv(h, s, value, nowait=nowait)

    async def set_colourtemp_percentage(self, colourtemp=100, nowait=False):
        """
        ...
        c = int(self.dpset['value_max'] * colourtemp // 100)
        return await self.set_colourtemp( c, nowait=nowait )

    # Deprecated.  Please use set_white_percentage() instead.
    async def set_colourtemp(self, colourtemp, nowait=False):
        """
    ...
    def colour_rgb(self, state=None, nowait=False):
        ...
        return BulbDeviceAsync.hexvalue_to_rgb(hexvalue, self.dpset['value_hexformat'])
