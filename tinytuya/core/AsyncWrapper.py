...
class AsyncWrapper:
    ...
    def __del__(self):
        """
        __del__ has the potential to block a thread, it is best to explicitly call a cleanup method.
        However, if the object is garbage collected without this explicit call,
        the system will attempt to perform the cleanup without waiting for the coroutine to complete.
        """
            ...
            if hasattr(self._async_impl, 'close'):
                try:
                    ...
                    if self._is_async_method(close_method):
                        self._runner.run(close_method(), nowait)
                    else:
                        close_method()
                except Exception:
